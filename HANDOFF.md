...
- 决策次数上限 `maxDecisions = 80` 在一局后期就用完了，郑泌昌被拿问后没有触发最后决策。被革职的人最后那次决策不受上限约束；上限改成成本护栏，数值按实玩调整（一局约 86 次调用、134 万 token）。
- 锦衣卫回奏：淹田数应该报受淹面积，而不是剩余淹田（买田会减少 `floodedMu`）；“官府放赈”要和商人以粮换田分开统计。
